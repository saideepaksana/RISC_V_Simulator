...
public:
    ...
    void c_read(long long memo, long long &u_data, int size) // we'll deal with sign later in load store
    {
        int xx = log(block_size);
        int yy = log(in);
        ...
        if (r_policy == 0)
        {
            ...
            if (!hit)                   // search for any invalid block in particular index
            {
                int invp = -1;
                for (int i = 0; i < assoc; i++)
                {
                ...
                else                // replace
                {
                    ...
                    block xy = p[index][tbri]; // dirty bit replacement
                    if (xy.dirty_bit)
                    {
                        for (int i = 0; i < block_size; i++)
        ...
        else if (r_policy == 2)
        {
            ...
            if (!hit) // search for any invalid block in particular index
            {
                ...
                if (invp != -1)
                {
                    ...
                    u_data = 0;
                    for (int i = 0; i < size; i++)
                        u_data += (mem[memo + i - 0x10000] << (8 * i)); // write allocate is always being use
                }
                else // replace
                {
                    long long ta = (memo >> log(in)) >> log(block_size);
        ...
        if (w_policy == 2)
        {
            ...
            if (!hit)
            {
                ...
                { // we replace invlid block first
                    ...
                    long long split = 255;
                    for (int i = 0; i < block_size; i++)
                        p[index][invp].m[i] = mem[(mem_tag << log(in) << (log(block_size))) + index * block_size + i];
                    for (int t = start; t <= stop; t++)
                    {
                ...
                else
                {
                    if (r_policy != 0)
                    {
                        for (int tbri = 0; tbri < assoc; tbri++)
                        {
                            ...
                            {
                                block xy = p[index][tbri]; // dirty bit  helped replacement
                                if (xy.dirty_bit)
                                {
                                    for (int i = 0; i < block_size; i++)
                                    {
                                ...
                                long long split = 255;
                                for (int i = 0; i < block_size; i++)
                                    p[index][tbri].m[i] = mem[(mem_tag << log(in) << (log(block_size))) + index * block_size + i];
                                for (int t = start; t <= stop; t++)
                                {
                                    p[index][tbri].m[t] = (split << (8 * count)) & u_data;
                                    count++;
                                }
                                for (int i = 0; i < assoc; i++)
                                {
...
void execute(string a) // Function that executes the given RISC-V instruction in string a.
{
    ...
    else if (ins == "bne") // For Branch instructions.
    {
...
int main()
{
    ...
    while (1) // Runs in an infinite loop
    {
        ...
        if (inst == "cache_sim")
        {
            ...
            if (x == "enable")
            {
                if (program_completion == 1 || v.size() == 0)
                {
                    cache_enable = 1;
                    ifstream f(x);
                    string s;
            ...
            if (x == "status")
            {
                if (cache_enable)
                {
                    ...
                    cout << "Cache Size: " << cp->capacity << endl
                         ...
                         << "Replacement Policy: " << cp->r_policy << endl
                         << "Write Back Policy: " << cp->w_policy << endl;
                }
                else
                {
        ...
        else if (inst == "s") // For the "step" command.
        {
        ...
        else if (inst == "b") // For the "break" commnd.
        {
            int b;
